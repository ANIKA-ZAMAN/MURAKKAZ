...
const initialFavorites: WishlistProduct[] = productsCatalog.slice(0, 8).map((p, idx) => ({
  id: `fav-${idx + 1}`,
  name: p.name,
  image: p.image || "/images/products/jade_serenity.png",
  inspiredBy: p.inspiredBy ? `Inspired by ${p.inspiredBy}` : (p.brand ? `By ${p.brand}` : p.name),
  price: "300 - 2500tk",
  rating: (p.rating || 4.8).toString(),
  ratingCount: p.reviews || 180,
  inWishlist: true,
}));

const initialRelated: WishlistProduct[] = productsCatalog.slice(8, 16).map((p, idx) => ({
  id: `rel-${idx + 1}`,
  name: p.name,
  image: p.image || "/images/products/jade_serenity.png",
  inspiredBy: p.inspiredBy ? `Inspired by ${p.inspiredBy}` : (p.brand ? `By ${p.brand}` : p.name),
