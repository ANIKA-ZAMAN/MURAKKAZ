...
export default function ProductDetailsClient({ id, initialProduct }: { id: string; initialProduct?: any }) {
  ...
  const [liveProduct, setLiveProduct] = useState<any>(() => {
    if (initialProduct) {
      const hasDbImage = initialProduct.image && typeof initialProduct.image === 'string' && initialProduct.image.trim().length > 0 && !initialProduct.image.includes('jade_serenity.png');
      const resolvedImage = hasDbImage ? initialProduct.image : (initialProduct.image || '/images/products/jade_serenity.png');
      const rawGallery = Array.isArray(initialProduct.galleryImages)
        ? initialProduct.galleryImages.map((g: any) => typeof g === 'string' ? g : g?.url).filter((u: any) => typeof u === 'string' && u.trim().length > 0)
        : [];
